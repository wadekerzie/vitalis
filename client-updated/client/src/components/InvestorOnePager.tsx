      ...
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div {...fadeIn} className="mb-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <MetricBlock label="Startup Capital" value="$1.07M" sub="Build-out + equipment + AI deployment" />
        ...
        <motion.div {...fadeIn} className="mb-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
            {[
              ...
              "Surgical Recovery Programs",
              "Executive Longevity Physicals",
            ].map((svc) => (
              <div key={svc} className="p-3 rounded-lg bg-white/5 border border-white/10 text-center">
                <span className="text-xs font-semibold text-white/70 leading-snug block">{svc}</span>
              </div>
            ))}
          </div>
          <div className="mt-3 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 text-xs text-blue-300 text-center">
            All services delivered under a proprietary 30-day optimization cycle — the Vitalis 30 Engine. Members don't buy sessions. They buy a system.
          </div>
        </motion.div>
        ...
        <motion.div {...fadeIn} className="mb-10 grid md:grid-cols-2 gap-6">
          <div>
            <SectionLabel>Use of Capital — Flagship ($1,065,400 Total)</SectionLabel>

            {/* Bucket 1: Physical */}
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold uppercase tracking-widest text-blue-400">Physical Infrastructure</span>
                <span className="text-xs font-bold text-blue-400">$655,400</span>
              </div>
              <div className="space-y-1 pl-2 border-l-2 border-blue-500/30">
                {[
                  { item: "Build-out (2,500 sqft @ $120/sqft)", amount: "$300,000" },
                  { item: "Medical Equipment (IV chairs, DEXA, InBody, cryo, red light)", amount: "$235,400" },
                  { item: "Furniture, Fixtures & Finishes", amount: "$40,000" },
                  { item: "Lab Equipment & Infusion Pumps", amount: "$45,400" },
                  { item: "EMR Hardware & On-Premise Infrastructure", amount: "$35,000" },
                ].map((row) => (
                  <div key={row.item} className="flex justify-between items-start py-1.5 border-b border-white/5 text-xs gap-2">
                    <span className="text-white/50 leading-snug">{row.item}</span>
                    <span className="font-semibold text-white whitespace-nowrap">{row.amount}</span>
                  </div>
                ))}
              </div>
            </div>

            {/* Bucket 2: Expertise & IP */}
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold uppercase tracking-widest text-emerald-400">Expertise, IP & Operations</span>
                <span className="text-xs font-bold text-emerald-400">$410,000</span>
              </div>
              <div className="space-y-1 pl-2 border-l-2 border-emerald-500/30">
                {[
                  { item: "AI Architecture & Deployment — Kerzie AI (Phase 1)", amount: "$200,000" },
                  { item: "Working Capital (3 months operating runway)", amount: "$120,000" },
                  { item: "Pre-Opening Marketing & Brand Launch", amount: "$75,000" },
                  { item: "Legal, Licensing & HIPAA Compliance", amount: "$35,000" },
                ].map((row) => (
                  <div key={row.item} className="flex justify-between items-start py-1.5 border-b border-white/5 text-xs gap-2">
                    <span className="text-white/50 leading-snug">{row.item}</span>
                    <span className="font-semibold text-white whitespace-nowrap">{row.amount}</span>
            ...
            <div className="flex justify-between items-center py-2 text-sm font-bold border-t border-white/20 mt-2">
              <span className="text-white">Total Startup Capital</span>
              <span className="text-emerald-400 text-base">$1,065,400</span>
            </div>
          </div>
          <div>
        ...
        <motion.div {...fadeIn} className="mb-10">
          ...
          <div className="p-5 rounded-xl bg-white/5 border border-white/10 flex flex-col md:flex-row gap-4 items-start">
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-emerald-500 flex items-center justify-center text-white font-bold text-lg flex-shrink-0">MD</div>
            <div>
              <div className="font-bold text-white text-lg">Medical Director — To Be Contracted</div>
              <div className="text-sm text-white/50 mb-2">Part-time oversight, Dallas market</div>
              <p className="text-sm text-white/60 leading-relaxed">
                The Medical Director role is a defined operational requirement, not a contingency. A part-time physician provides clinical oversight, protocol development authority, and the credentialing required for hormone protocols and advanced lab interpretation. This structure — part-time Medical Director supported by full-time RNs — is the standard model for high-performing longevity clinics and is fully reflected in the proforma at $12,500/month.
              </p>
            </div>
          </div>
        </motion.div>
